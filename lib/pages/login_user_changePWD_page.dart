...
class _LoginUserChangePWDPageState extends State<LoginUserChangePWDPage> {
  ...
  Future<void> _changePassword() async {
    ...
    if (newPassword.length < 4) {
      setState(() => newPasswordError = "4자 이상으로 설정해 주세요.");
      return;
    }

    // 동일 비밀번호 금지
    if (newPassword == currentPassword) {
      setState(() => newPasswordError = "이전과 동일한 비밀번호는 사용할 수 없습니다.");
      return;
    }

    // 확인 불일치
    if (newPassword != confirmPassword) {
      setState(() => confirmPasswordError = "새 비밀번호가 서로 일치하지 않습니다.");
      return;
    }
    ...
    try {
      // ✅ 서버 규격에 맞게 old_password로 전송
      final response = await HttpClient.patchUserUpdate(
        token: token,
        data: {
          "password": {
            "old_password": currentPassword, // ← 여기!
      ...
      if (serverMsg.contains("현재 비밀번호가 틀립니다") || serverMsg.contains("old_password")) {
        setState(() => currentPasswordError = "현재 비밀번호가 올바르지 않습니다.");
      } else if (serverMsg.contains("8자") || serverMsg.contains("길이") || serverMsg.contains("복잡도")) {
        // 백엔드가 더 엄격한 정책을 요구하면 그대로 노출
        setState(() => newPasswordError = serverMsg);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text("새로운 비밀번호를 입력해 주세요", style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            ...
            _buildPasswordField(
              ...
              toggleVisibility: () => setState(() => showCurrentPassword = !showCurrentPassword),
              icon: Icons.verified_user_outlined,
              errorText: currentPasswordError,
              onChanged: (_) {
                if (currentPasswordError != null) setState(() => currentPasswordError = null);
              },
            ),
            ...
            _buildPasswordField(
              ...
              toggleVisibility: () => setState(() => showNewPassword = !showNewPassword),
              icon: Icons.lock,
            ...
            if (_newPasswordController.text.isNotEmpty && newPasswordError == null)
              Padding(
                padding: const EdgeInsets.only(top: 8, left: 20),
                child: Align(
                  alignment: Alignment.centerLeft,
                  child: Text(
                    _passwordStrengthLabel(_newPasswordController.text),
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: Colors.red.withOpacity(0.15), blurRadius: 8, offset: Offset(0, 4))],
          ),
          child: TextField(
